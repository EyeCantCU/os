...
# While in other c3 containers we would stop the database and run it in foreground,
...
if is_cassandra_running; then
    ...
    if am_i_root; then
        exec gosu "$DAEMON_USER" "${__run_tail_cmd}" "${__run_tail_flags[@]}"
    else
    ...
    if am_i_root; then
        exec gosu "$DAEMON_USER" "${__run_cmd}" "${__run_flags[@]}"
    else
