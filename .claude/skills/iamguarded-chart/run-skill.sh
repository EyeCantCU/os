...
#
# Wrapper script for iamguarded-chart skill
#
...
# 3. Adds required Bash permissions for the skill
# 4. Launches Claude with the skill (optionally with application name)
# 5. Cleans up permissions and MCP registration on exit
#

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SERVER_PATH="${SCRIPT_DIR}/server.py"
REQUIREMENTS_PATH="${SCRIPT_DIR}/requirements.txt"
MCP_SERVER_NAME="iamguarded-chart"

# Settings file path (in the skill directory)
...
if [[ -n "${APPLICATION_NAME}" ]]; then
  ...
  claude --settings "${SETTINGS_FILE}" "Use the /iamguarded-chart skill to create an iamguarded chart for ${APPLICATION_NAME}"
else
