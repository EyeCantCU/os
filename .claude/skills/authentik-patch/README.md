...
A Claude Code skill that automates the process of
updating authentik package versions and regenerating
enterprise module removal patches.

## Overview

Authentik's upstream repository includes enterprise
features that are loaded unconditionally despite
licensing restrictions. The stereo repository maintains
OSS builds by applying patches that remove all
`authentik.enterprise` module references.

This skill automates the tedious manual work required
for each version update:

- Automatically finds enterprise references using
  ripgrep
- Compares findings with existing patches
- Identifies new files needing patches and removed
  files
- Generates patch templates with context
- Validates patches apply cleanly
- Updates package YAML files with correct
  version/commit
- Guides through build and test process

**Why this is needed**: Per
[upstream discussion #18682][disc-18682], authentik's
enterprise code is loaded unconditionally and this
...
- **Working directory**: Must run from the stereo
...
Claude will launch and immediately start the update
process for version 2025.12.3.
...
├── run-skill.sh          # Wrapper script
├── requirements.txt      # Python dependencies
├── README.md             # This file
└── references/
    └── patch-patterns.md # 15 patching patterns
```

### MCP Tools

The skill provides 9 specialized tools accessible
during the workflow:

1. **checkout_authentik** - Clone and checkout version
2. **list_upstream_versions** - Show recent versions
3. **find_enterprise_refs** - Search for enterprise refs
4. **analyze_patch_coverage** - Compare with patches
5. **generate_patch_template** - Create patch templates
6. **validate_patches** - Test if patches apply cleanly
7. **sync_patches** - Sync patches to authentik-fips/
8. **update_package_yaml** - Update version/commit
9. **cleanup** - Remove temporary directories
...
- Select packages to update
  (authentik, authentik-fips, or both)
...
#### Phase 3.5: Sync Patches to FIPS (scope: both)

- Sync finalized patches to authentik-fips/
- Validate FIPS patches apply cleanly
...
| ------------- | -------------- |
| Patch version | 10-15 minutes  |
| Minor version | 20-30 minutes  |
| Major version | 45-60 minutes  |
...
MCP server: .claude/skills/authentik-patch/server.py
Working directory: /path/to/stereo
Registering MCP server...
Launching Claude with version: 2025.12.3

# Claude starts and runs the skill...
# Phase 1: Checks current version, confirms update
# Phase 2: Checks out upstream, finds enterprise refs
#          Analysis shows coverage report
# Phase 3: Generates templates, user creates patches
#          Validates all patches apply cleanly
# Phase 4: Updates YAML files
# Phase 5: Builds, tests pass, commits changes
...
"ERROR: Must be run from enterprise-packages
repository root"

**Solution**: Navigate to the stereo repository root.

```bash
cd /path/to/stereo
pwd  # Verify you're in the right place
ls enterprise-packages/  # Should list packages
```
...
1. Review error messages to identify failing patches
2. Check if upstream file structure changed
3. Use `generate_patch_template` for current state
4. Manually adjust patch with correct line numbers
5. See `references/patch-patterns.md` for guidance
...
1. **Missed enterprise reference**: Error mentions
   missing import
   - Re-run `find_enterprise_refs`
   - Add missing reference to appropriate patch

2. **Patch application failed**: Build can't apply
   a patch
   - Validate with `validate_patches` tool
   - Fix failing patch files

3. **Dependency changes**: Upstream changed
   requirements
...
uv run \
    --with-requirements \
    .claude/skills/authentik-patch/requirements.txt \
...
claude \
    "Use authentik-patch's list_upstream_versions tool"

# Checkout a version
claude \
    "Use authentik-patch's checkout_authentik tool \
    with version 2025.12.3"
```

### Test Full Workflow

Run through complete workflow with a test version:
...
If you need to update patches but keep the same
version:
...
1. Create patch in `enterprise-packages/authentik/`
...
See `references/patch-patterns.md` for documentation
of 15 common patterns:
...
9. Comment out enterprise provider API calls
...
15. Stub enterprise-only API response fields
...
- **YAML formatting**: Uses `ruamel.yaml` to preserve
  comments
- **Linting**: Reminds to run `./lint.sh` on YAML
- **Testing**: Integrates with
  `make package/authentik` and
  `make test/authentik`
- **Guardrails**: Follows patterns from `CLAUDE.md`
...
- **Stereo repo conventions**: `CLAUDE.md`
- **Current patches**:
...
This skill is part of the stereo enterprise-packages
repository tooling.
